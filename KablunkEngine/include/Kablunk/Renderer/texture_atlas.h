...
struct TextureAtlasSprite
{
	...
	Kablunk::ref<texture_atlas> texture_atlas;
};

class texture_atlas : public Kablunk::RefCounted
{
public:
    texture_atlas(const std::string& filepath, u32 width = 128ul, u32 height = 128ul);
    texture_atlas(const std::vector<ref<Kablunk::Texture2D>> textures, u32 width = 128ul, u32 height = 128ul);
    ...
    // get sprite by uuid
    TextureAtlasSprite get_texture_by_uuid(Kablunk::uuid::uuid64 uuid) const;
    
    // check if uuid is inside sprite map
    bool is_uuid_in_sprite_map(Kablunk::uuid::uuid64 uuid) const { return m_sprite_map.find(uuid) != m_sprite_map.end(); }
    ...
    ref<Kablunk::Texture2D> get_texture_atlas() { return m_texture_atlas; }
    // get an immutable reference to the texture atlas
    const ref<Kablunk::Texture2D>& get_texture_atlas() const { return m_texture_atlas; }
...
private:
    ...
    void create_texture_atlas_from_images(const std::vector<ref<Kablunk::Texture2D>> textures);
private:
    // primary atlas texture that is used to create sub sprites
    ref<Kablunk::Texture2D> m_texture_atlas;
    ...
    std::unordered_map<Kablunk::uuid::uuid64, TextureAtlasSprite> m_sprite_map;
