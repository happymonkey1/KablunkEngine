...
public:
    ...
    [[deprecated]] void add_font_file_to_library(arc<font_asset_t> font_asset, const std::filesystem::path& p_absolute_filepath);
	// check if a reference to the given font asset is in the cache
    [[deprecated]] bool has_font_cached(arc<font_asset_t> font_asset) const;
	// try to remove a font from the font cache
    [[deprecated]] void remove_font_file_from_library(arc<font_asset_t> font_asset);
	// try get a font by filename
    [[deprecated]] arc<font_asset_t> get_font_asset(asset::asset_id_t p_font_asset_id) const;
    ...
    [[deprecated]] const kb::unordered_flat_map<asset::asset_id_t, arc<font_asset_t>>& get_font_cache() const { return m_font_cache; }

    // get default font loaded by the engine
    [[nodiscard]] auto get_default_font() const noexcept -> kb::arc<render::font> { return m_default_font; }
    // get the default monospaced font loaded by the engine
    [[nodiscard]] auto get_default_monospaced_font() const noexcept -> kb::arc<render::font> { return m_default_monospaced_font; }
private:
	// cache of fonts loaded into memory
	// maps font filename to font assets
	kb::unordered_flat_map<asset::asset_id_t, arc<font_asset_t>> m_font_cache;
	// underlying font rendering engine (freetype)
	FT_Library m_ft_library = nullptr;
    // default font
    arc<font> m_default_font = arc<font>::Create("resources/fonts/inter/Inter-Regular.ttf");
    // defualt monospaced font
    arc<font> m_default_monospaced_font = arc<font>::Create("resources/fonts/jetbrains-mono/JetBrainsMono-Regular.ttf");
};
