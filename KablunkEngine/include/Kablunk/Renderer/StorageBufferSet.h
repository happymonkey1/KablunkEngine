...
namespace kb
{
	...
	public:
		...
		virtual arc<StorageBuffer> Get(uint32_t binding, uint32_t set, uint32_t frame) = 0;
		virtual void Set(arc<StorageBuffer> storage_buffer, uint32_t set = 0, uint32_t frame = 0) = 0;
		virtual void Resize(uint32_t binding, uint32_t set, uint32_t new_size) = 0;

		static arc<StorageBufferSet> Create(uint32_t frames);
	};
