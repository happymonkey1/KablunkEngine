...
struct renderer_2d_data_t
{
    ...
    arc<backend::shader> quad_shader;
    arc<backend::shader> circle_shader;
    arc<backend::shader> line_shader;
    arc<backend::shader> ui_shader;
	arc<backend::shader> text_shader;

	// =======

    arc<backend::texture_2d> white_texture;
    ...
    arc<backend::render_pass> m_line_pass;
    arc<backend::render_pass> m_ui_pass;
	arc<backend::render_pass> m_text_pass;

    // --- materials ----------------------------
    arc<backend::material> quad_material;
    arc<backend::material> circle_material;
    arc<backend::material> line_material;
    arc<backend::material> ui_material;
	arc<backend::material> text_material;
...
class renderer_2d : public RefCounted
{
public:
	renderer_2d() = default;
    ...
    [[nodiscard]] inline auto submit_texture(const arc<backend::texture_2d>& p_texture) noexcept -> render::renderer_2d_texture_id
    {
        //KB_PROFILE_SCOPE;
