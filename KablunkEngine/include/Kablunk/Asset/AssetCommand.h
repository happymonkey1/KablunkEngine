...
template <typename T>
arc<T> get_asset(const std::filesystem::path& filepath)
{
...
// try find an asset by filepath
template <typename T>
arc<T> get_asset(const char* filepath_cstr)
{
...
arc<IAsset> create_asset(const std::filesystem::path& filepath, const std::filesystem::path& directory_path, Args&&... args)
{
	return Singleton<AssetManager>::get().create_asset<T>(filepath, directory_path, args);
}

template <typename T, typename... Args>
arc<IAsset> create_asset(const std::filesystem::path& filepath, Args&&... args)
{
