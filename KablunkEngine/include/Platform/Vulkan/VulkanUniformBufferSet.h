...
namespace Kablunk
{
	...
	public:
		...
		virtual IntrusiveRef<UniformBuffer> Get(uint32_t binding, uint32_t set = 0, uint32_t frame = 0) override;
		virtual void Set(IntrusiveRef<UniformBuffer> uniform_buffer, uint32_t set = 0, uint32_t frame = 0) override;
	private:
		uint32_t m_frames;
		std::map<uint32_t, std::map<uint32_t, std::map<uint32_t, IntrusiveRef<UniformBuffer>>>> m_uniform_buffers;
	};
}
