#pragma once
#ifndef KABLUNK_PLATFORM_VULKAN_STORAGE_BUFFER_SET_H
#define KABLUNK_PLATFORM_VULKAN_STORAGE_BUFFER_SET_H

#include "Kablunk/Renderer/StorageBufferSet.h"

#include <map>

namespace kb
{ // start namespace kb
class VulkanStorageBufferSet final : public StorageBufferSet
{
public:
	explicit VulkanStorageBufferSet(uint32_t frames);
	virtual ~VulkanStorageBufferSet() override = default;

	virtual void Create(uint32_t size, uint32_t binding) override;

	virtual arc<StorageBuffer> Get(uint32_t binding, uint32_t set, uint32_t frame) override;
	virtual void Set(arc<StorageBuffer> storage_buffer, uint32_t set = 0, uint32_t frame = 0) override;
	virtual void Resize(uint32_t binding, uint32_t set, uint32_t new_size) override;
private:
	uint32_t m_frames;
	std::map<uint32_t, std::map<uint32_t, std::map<uint32_t, arc<StorageBuffer>>>> m_storage_buffers;
};
} // end namespace kb

#endif
