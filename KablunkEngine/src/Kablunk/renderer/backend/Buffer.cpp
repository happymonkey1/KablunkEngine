...
arc<VertexBuffer> VertexBuffer::Create(uint32_t size)
{
	...
	case backend::render_backend_type_t::vulkan:
        return static_cast<arc<VertexBuffer>>(arc<backend::vk::vulkan_vertex_buffer>::Create(size));
    default:
    {
        KB_CORE_ASSERT(
            false,
...
arc<VertexBuffer> VertexBuffer::Create(const void* data, uint32_t size)
{
    ...
    {
    case backend::render_backend_type_t::vulkan:
        return static_cast<arc<VertexBuffer>>(arc<backend::vk::vulkan_vertex_buffer>::Create(data, size));
    default:
    {
        KB_CORE_ASSERT(
            false,
