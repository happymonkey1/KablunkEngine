...
vulkan_compute_pipeline::vulkan_compute_pipeline(arc<shader> compute_shader)
    : m_shader{ compute_shader.As<vulkan_shader>() }
{
    render::submit(
    ...
    register_shader_dependency(compute_shader, arc<render::backend::compute_pipeline>{ this });
}

void vulkan_compute_pipeline::execute(VkDescriptorSet* p_descriptor_sets, uint32_t p_descriptor_set_count, uint32_t p_group_count_x, uint32_t p_group_count_y, uint32_t p_group_count_z)
{
    const VkDevice vk_device = vulkan_context::get()->get_device()->get_vk_device();
    const VkQueue vk_compute_queue = vulkan_context::get()->get_device()->get_vk_compute_queue();
    const VkCommandBuffer vk_compute_command_buffer = vulkan_context::get()->get_device()->get_vk_command_buffer(
        true,
        true
    );
...
void vulkan_compute_pipeline::begin(arc<render_command_buffer> p_render_command_buffer /*= nullptr*/)
{
    ...
    {
        m_vk_active_command_buffer = vulkan_context::get()->get_device()->get_vk_command_buffer(true, true);
...
void vulkan_compute_pipeline::rt_begin(arc<render_command_buffer> p_render_command_buffer /*= nullptr*/)
{
...
void vulkan_compute_pipeline::end()
{
    ...
    {
        const auto vk_compute_queue = vulkan_context::get()->get_device()->get_vk_compute_queue();
...
void vulkan_compute_pipeline::dispatch(const glm::uvec3& p_work_group_count) const
{
...
void vulkan_compute_pipeline::set_push_constants(kb::owning_buffer p_constants)
{
...
void vulkan_compute_pipeline::create_pipeline()
{
    kb::render::submit(
        [instance = kb::arc{ this }]() mutable
        {
...
void vulkan_compute_pipeline::rt_create_pipeline()
{
