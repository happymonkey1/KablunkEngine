...
bool shader_cache::has_changed(const std::filesystem::path& shader, const std::string& source)
{
...
void shader_cache::serialize(const std::map<std::string, uint32_t>& shader_cache)
{
	YAML::Emitter out;
	out << YAML::BeginMap;
...
void shader_cache::deserialize(std::map<std::string, uint32_t>& shader_cache)
{
