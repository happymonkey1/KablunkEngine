...
namespace Kablunk
{
		...
		: m_texture_width{ width }, m_texture_height{ height }
	{
		...
		KB_CORE_ASSERT(textures[0]->GetFormat() == ImageFormat::RGBA, "only RGBA image format is supported!");
	...
	void TextureAtlas::create_texture_atlas_from_images(const std::vector<IntrusiveRef<Texture2D>> textures)
	{
		...
		m_atlas_texture = Texture2D::Create(textures[0]->GetFormat(), final_atlas_width, final_atlas_width, atlas_image_data_buffer.get());
