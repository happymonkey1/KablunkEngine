...
namespace Kablunk
{
	...
	IntrusiveRef<Image2D> Image2D::Create(ImageSpecification specification, Buffer buffer)
	{
		...
		{
		case RendererAPI::RenderAPI_t::OpenGL:	KB_CORE_ASSERT(false, "OpenGL images not supported yet!"); return nullptr;
		case RendererAPI::RenderAPI_t::Vulkan:	return IntrusiveRef<VulkanImage2D>::Create(specification);
