	...
	: m_specification{std::move(spec)}
{
    ...
    if (!m_specification.m_existing_frame_buffer)
    {
        ...
        for (auto& attachment_specification : m_specification.m_attachments.Attachments)
		{
			...
			{
				ImageSpecification image_create_spec;
                image_create_spec.format = attachment_specification.format;
                image_create_spec.usage = ImageUsage::Attachment;
				...
				m_depth_attachment_image = Image2D::Create(image_create_spec);
			}
			else
			{
				ImageSpecification image_create_spec;
				...
				m_attachment_images.emplace_back(Image2D::Create(image_create_spec));
			}
			attachment_index++;
		}
	}
