...
namespace Kablunk
{
	...
	void VulkanStorageBufferSet::Create(uint32_t size, uint32_t binding)
	{
		...
		{
			IntrusiveRef<StorageBuffer> storage_buffer = StorageBuffer::Create(size, binding);
			Set(storage_buffer, 0, frame); // #TODO only one set currently supported!
		}
	}

	IntrusiveRef<StorageBuffer> VulkanStorageBufferSet::Get(uint32_t binding, uint32_t set, uint32_t frame)
	{
	...
	void VulkanStorageBufferSet::Set(IntrusiveRef<StorageBuffer> storage_buffer, uint32_t set /*= 0*/, uint32_t frame /*= 0*/)
	{
