...
void VulkanPipeline::RT_Invalidate()
{
	...
	pipeline_create_info.renderPass = frame_buffer->GetVkRenderPass();
	...
	size_t color_attachment_count = frame_buffer->get_specification().m_swap_chain_target ? 1 : frame_buffer->GetColorAttachmentCount();
