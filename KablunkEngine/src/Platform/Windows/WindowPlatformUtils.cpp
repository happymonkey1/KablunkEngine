...
auto FileSystem::read_bytes(const std::filesystem::path& p_filepath) noexcept -> owning_buffer
{
    std::ifstream file_stream{ p_filepath, std::ios::binary | std::ios::ate };
    KB_CORE_ASSERT(file_stream, "[WindowsPlatformUtil]: read_bytes({}) failed to open file!", p_filepath.string().c_str());

    const auto end = file_stream.tellg();
    file_stream.seekg(0, std::ios::beg);
    const auto size = end - file_stream.tellg();
    KB_CORE_ASSERT(size > 0, "[WindowsPlatformUtil]: {} is zero bytes?", p_filepath.string().c_str());

    owning_buffer buffer{ static_cast<size_t>(size) };
    file_stream.read(static_cast<char*>(buffer.get()), buffer.size());
    file_stream.close();
