...
namespace Kablunk
{
	...
	public struct Matrix4
	{
		...
		{
			LogMessage_Native(string.Format("{0:0.00}  {1:0.00}  {2:0.00}  {3:0.00}", D00, D10, D20, D30));
			LogMessage_Native(string.Format("{0:0.00}  {1:0.00}  {2:0.00}  {3:0.00}", D01, D11, D21, D31));
			LogMessage_Native(string.Format("{0:0.00}  {1:0.00}  {2:0.00}  {3:0.00}", D02, D12, D22, D32));
			LogMessage_Native(string.Format("{0:0.00}  {1:0.00}  {2:0.00}  {3:0.00}", D03, D13, D23, D33));
		}

		[MethodImpl(MethodImplOptions.InternalCall)]
		internal static extern ulong LogMessage_Native(string message);
	}
